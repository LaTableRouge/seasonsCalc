import { Equinox, EquinoxDates, Season, YearTables } from './types';
...
const isDateValid = (date: Date): boolean => date instanceof Date && !isNaN(date.getTime());

/* Meeus, page 177 */
const calculateJDE0 = (year: number, equinox: Equinox, yearTables: YearTables): number => {
...
const calculateS = (T: number): number => {
...
const JDEtoTimestamp = (JDE: number): number => {
    let tmp = JDE;
    tmp -= 2440587.5;
    tmp *= 86400;
...
const convertToDate = (JDE0: number): Date => {
...
const calculateSeason = (now: Date, equinoxDates: EquinoxDates): Season => {
...
const seasonsCalc = (now: Date = new Date()): Season => {
    if (!isDateValid(now)) {
        throw new Error('Invalid date object');
    }

    const equinoxDates: EquinoxDates = {} as EquinoxDates;
    const currentYear = now.getFullYear();

    const equinoxes: Equinox[] = ['mar', 'jun', 'sep', 'dec'];

    /* Tables from Meeus, page 178 */
    const yearTables: YearTables = {
...
export default seasonsCalc;

// Support for multiple module systems
